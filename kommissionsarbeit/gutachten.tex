...
Dieser Abschnitt soll dir ein paar Hilfestellungen geben, wie man pro oder contra bestimmter Bewerber\gstar{innen} argumentieren kann.

Auch wenn in machen Gutachten steht, dass die Lehrqualität des\gstar{der} Bewerber\gstar{in} hervorragend ist, so sind diese Gutachten immer mit Vorsicht zu genießen. Oft haben die Gutachter\gstar{innen} die einzelnen Bewerber\gstar{innen} nur auf Tagungen vortragen gesehen und kennen daher die Qualität ihrer Vorlesungen nicht.

\paragraph{Über die Gutachten}
\begin{itemize}
    \item Es gibt \emph{Einzelgutachten}, welche nur eine\gstar{n} Bewerber\gstar{in} begutachten. Diese fallen üblicherweise sehr gut bis hervorragend aus. Einzig ein neutrales bis negatives Einzelgutachten kann dir zur Entscheidungsfindung dienen.
    \item \emph{Vergleichende Gutachten} sind Gutachten, die mehrere der Bewerber\gstar{innen} vergleichen. Hierbei gibt der\gstar{die} Gutachter\gstar{in} meist auch seine\gstar{ihre} persönliche Reihung an. Dieses sind mit die wichtigsten Entscheidungshilfen.
    \item Wenn ein\gstar{e} Bewerber\gstar{in} \emph{nicht habilitiert} ist, muss im Gutachten stehen, dass er\gstar{sie} zu eigenständiger Lehre befähigt ist bzw. alle Voraussetzungen für eine Professur erfüllt.
    \item Die didaktische Eignung eines\gstar{r} Bewerbers\gstar{in} kann nur in den seltensten Fällen mit einem Standardgutachten begründet werden.
    \item Von den Bewerbern\gstar{innen} selbst vorgeschlagene Gutachter\gstar{innen} sollten mit Vorsicht genossen werden. Dieses sind meist Kandidaten\gstar{innen} für das Schreiben von Einzelgutachten. Einzelne Berufungsordnungen schließen von den Bewerbern\gstar{innen} vorgeschlagene Gutachter\gstar{innen} grundsätzlich aus.
    \item Jede\gstar{r} Bewerber\gstar{in} sollte von wenigstens zwei Gutachtern\gstar{innen} begutachtet werden, davon mindestens bei einem\gstar{r} Gutachter\gstar{in} vergleichend.
\end{itemize}

\paragraph{Argumentation gegen eine\gstar{n} Bewerber\gstar{in}}
\begin{itemize}
    \item Anhand des Kriterienkatalogs (hier muss pädagogische Eignung drin stehen, vgl. Seite~\pageref{kriterienkatalog}).
    \item Ggf. hat der\gstar{die} Bewerber\gstar{in} keine Habilitation.
